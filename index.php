...
if ($action === 'get_cursos') {
 $db = getDB();
 $rows = $db->query("SELECT id_curso,nombre,descripcion,fecha_inicio,fecha_fin,Requisitos,costo,Imagen_url FROM Curso ORDER BY fecha_inicio DESC LIMIT 20");
 $data = [];
 while ($r = $rows->fetch_assoc()) $data[] = $r;
 jsonOut($data);
}
...
if ($action === 'crear_curso' && isAdmin()) {
 ...
 $img = subirImagen('imagen_file');
 if (!$img) $img = trim($_POST['imagen_url'] ?? '');
 $stmt = $db->prepare("INSERT INTO Curso (nombre,descripcion,fecha_inicio,fecha_fin,id_usuario,Requisitos,costo,Imagen_url) VALUES (?,?,?,?,?,?,?,?)");
 if (!$stmt) {
 ...
 $stmt->bind_param('ssssisds', $nombre, $desc, $fi, $ff, $uid, $req, $costo, $img);
 }
 jsonOut(['ok' => $stmt->execute(), 'id' => $db->insert_id]);
}
...
if ($action === 'editar_curso' && isAdmin()) {
 ...
 $img = subirImagen('imagen_file');
 if (!$img) $img = trim($_POST['imagen_url'] ?? '');
 $stmt = $db->prepare("UPDATE Curso SET nombre=?,descripcion=?,fecha_inicio=?,fecha_fin=?,Requisitos=?,costo=?,Imagen_url=? WHERE id_curso=?");
 if (!$stmt) {
 ...
 $stmt->bind_param('sssssdsi', $nombre, $desc, $fi, $ff, $req, $costo, $img, $id);
 }
 jsonOut(['ok' => $stmt->execute()]);
}
...
<head>
 ...
 <link rel="icon" href="Gemini_Generated_Image_x6gn39x6gn39x6gn" type='image/png' id='icon'>
...
.slide-track.is-paused {
 ...
 /* Pausar la animación mediante una clase que controlaremos con JS */
 .is-paused {
 animation-play-state: paused !important;
 }
 /* Contenedor sin bordes bruscos, muy limpio */
 .slider {
 width: 100%;
 height: 400px;
 overflow: hidden;
 ...
 .slide-track {
 display: flex;
 width: calc(var(--slide-width) * (var(--num-slides) * 2));
 animation: scroll var(--speed) linear infinite;
 }

 /* Pausar suavemente al tocar */
 ...
 100% { transform: translateX(calc(var(--slide-width) * var(--num-slides) * -1)); }
 }

 /* Estilo de Tarjeta Apple */
 .slide {
 width: var(--slide-width);
 height: 380px;
 padding: 0 12px; /* Separación sutil entre tarjetas */
 flex-shrink: 0;
 ...
 .card-inner:hover {
 transform: scale(1.02); /* Efecto táctil sutil */
 }
 ...
 /* Overlay gradiente estilo iOS para que el texto sea legible */
 .overlay {
 position: absolute;
 bottom: 0;
 left: 0;
 ...
 /* Desvanecido en los bordes de la pantalla */
 .slider::before,
 .slider::after {
 content: "";
 height: 100%;
 ...
 *::after {
 ...
 height: 44px;
 border-radius: 12px;
 background: linear-gradient(135deg, var(--blue), #f8960d);
 display: flex;
 align-items: center;
 justify-content: center;
 ...
 font-weight: 900;
 color: #fff;
 box-shadow: 0 4px 16px rgba(255, 202, 26, 0.35)
 }
 ...
 color: var(--text3);
 flex-shrink: 0;
 transition: left .35s cubic-bezier(.4,0,.2,1), all .2s;
 z-index: 200;
 box-shadow: 0 2px 8px rgba(26,86,255,.12)
 }
 ...
 .modal-img-container.sin-imagen {
 height: 120px;
 }
 ...
 .mob-ham:hover { background: var(--blue-soft); border-color: var(--blue-mid); }
 .sidebar-overlay {
 display: none;
 position: fixed;
 inset: 0;
 ...
 /* Tablet/Movil grande */
 @media(max-width:900px) {
 /* Ocultar botón hamburguesa del topbar (el bottom nav tiene el botón de menú) */
 ...
 .sidebar.mobile-open {
 transform: translateX(0);
 box-shadow: 8px 0 40px rgba(13,27,42,.28);
 }

 /* Forzar que etiquetas y títulos del sidebar sean siempre visibles en móvil */
 .sidebar .nav-label,
 .sidebar .sec-title,
 .sidebar .sb-foot-info,
 .sidebar .logo-text-wrap {
 opacity: 1 !important;
 pointer-events: auto !important;
 }
 .sidebar .nav-label { color: var(--text1) !important; font-size: 15px !important; }
 .sidebar .sec-title { color: var(--text3) !important; }
 .sidebar .nav-icon { font-size: 20px !important; }
 .sidebar .nav-item { padding: 13px 18px; margin: 3px 10px; }
 .sidebar .nav-item.active .nav-label { color: var(--blue) !important; }

 .main { margin-left: 0 !important; }

 .topbar { padding: 10px 12px; gap: 8px; flex-wrap: nowrap; }
 .topbar-title { font-size: 16px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 160px; }
 .topbar-sub { display: none; }

 .search-wrap { display: none; }
 .search-wrap.mob-visible { display: flex; }

 .topbar-right { gap: 6px; flex-shrink: 0; min-width: 0; }
 #topbarGuest { gap: 4px; }
 #topbarGuest button { font-size: 11px !important; padding: 7px 10px !important; white-space: nowrap; }

 .hero { padding: 22px 16px; margin-bottom: 18px; }
 .hero-h { font-size: clamp(30px, 8vw, 52px); }
 .hero-p { font-size: 13px; }
 .hero-kpis { gap: 14px; padding-top: 20px; margin-top: 20px; }
 .hkpi-val { font-size: 26px; }
 ...
 .insc-card { flex-direction: column; gap: 10px; }
 .insc-actions { flex-wrap: wrap; }

 .modal-box { width: 96vw; max-height: 88vh; }
 .m-info-grid { grid-template-columns: 1fr; }
 .m-body { padding: 16px; }
 .m-title { font-size: 22px; }

 /* Carrusel */
 .mi-carrusel .slide { min-width: 200px; }
 .mi-carrusel img { height: 200px; }
 /* El slider del dashboard */
 .slider { height: auto; padding: 10px 0; }
 .slider .slide { min-width: 200px; }
 .slider .card-inner img { height: 180px; }

 /* Admin */
 .admin-header { padding: 16px 14px; flex-direction: column; align-items: flex-start; gap: 12px; }
 .admin-title-wrap .admin-title { font-size: 24px; }
 .admin-tabs {
 width: 100%;
 overflow-x: auto;
 flex-wrap: nowrap;
 ...
 .adm-tab { font-size: 11px; padding: 7px 12px; white-space: nowrap; }
 .admin-content { padding: 12px 10px 80px; }
 ...
 .tbl-actions { display: flex; flex-direction: column; gap: 4px; }
 .tbl-actions button { font-size: 10px; width: 100%; }
 .tbl-actions select { font-size: 10px; width: 100%; }
 }
 ...
 .events-grid, .courses-grid, .media-grid { grid-template-columns: 1fr; }

 .topbar { padding: 8px 10px; gap: 6px; }
 .topbar-title { font-size: 14px; max-width: 130px; }

 /* En pantallas muy pequeñas solo mostrar "Iniciar Sesión", ocultar "Registrarse" */
 #topbarGuest button:nth-child(2) { display: none; }
 #topbarGuest button:nth-child(1) { font-size: 11px !important; padding: 7px 10px !important; }

 .hero { padding: 16px 12px; }
 .hero-chip { font-size: 9px; padding: 4px 10px; }
 .hero-h { font-size: clamp(26px, 9vw, 40px); }
 .hero-p { display: none; }
 .hero-kpis { flex-wrap: wrap; gap: 10px 20px; }

 .mi-carrusel .slide { min-width: 150px; }
 .mi-carrusel img { height: 170px; }
 .slider .slide { min-width: 150px; }
 .slider .card-inner img { height: 160px; }

 .event-card-header { padding: 12px 12px 8px; }
 ...
 .modal-box { width: 100vw; max-height: 92vh; border-radius: var(--r) var(--r) 0 0; }

 .adm-tab { font-size: 10px; padding: 6px 10px; }

 .insc-card { padding: 14px; }
 .insc-nombre { font-size: 14px; }

 .toast { left: 12px; right: 12px; bottom: 16px; max-width: 100%; }

 /* Bottom nav más compacto */
 .bn-item { min-width: 44px; padding: 5px 6px; }
 .bn-item .bn-icon { font-size: 18px; }
 .bn-item .bn-label { font-size: 8px; }
 }
 .logo-image {
 height: 40px;
 width: 40px;
 ...
 box-shadow: 0 -4px 20px rgba(26,86,255,.08);
 }
 .bottom-nav-inner {
 display: flex;
 ...
 .bn-item .bn-label { font-size: 9px; font-weight: 700; color: var(--text3); letter-spacing: .3px; text-transform: uppercase; }
 .bn-item.active .bn-label { color: var(--blue); }
 .bn-item.active .bn-icon { filter: drop-shadow(0 0 4px rgba(255,149,0,.5)); }

 @media(max-width:900px) {
 .bottom-nav { display: block; }
 /* espacio para que el contenido no quede tapado por el nav */
 .content { padding-bottom: 90px !important; }
 .admin-content { padding-bottom: 90px !important; }
 }

 /* Botón búsqueda móvil visible */
 @media(max-width:900px) {
...
<body>
 ...
 <img src="Gemini_Generated_Image_x6gn39x6gn39x6gn.png" alt="Logo" class="logo-image">
 ...
 <div class="nav-item ac tive" onclick="navigate('dashboard',this)">
 ...
 <button class="collapse-btn" onclick="toggleSidebar()" id="collapseBtn" title="Colapsar menú">◀</button>
 ...
 <!-- Buscador normal (escritorio) -->
 <div class="search-wrap" id="searchDesktop">
 <span style="color:var(--text3)">🔍</span>
 <input type="text" placeholder="Buscar eventos, cursos..." id="searchInput" oninput="handleSearch(this.value)">
 </div>
 <!-- Botón búsqueda móvil -->
 <button id="mobSearchBtn" onclick="toggleMobSearch()" style="display:none;width:36px;height:36px;border-radius:10px;background:var(--surface);border:1.5px solid var(--border);cursor:pointer;font-size:16px;align-items:center;justify-content:center;flex-shrink:0">🔍</button>
 <div id="topbarGuest" style="display:flex;gap:8px;align-items:center;flex-shrink:0">
 <button style="padding:8px 14px;border-radius:30px;background:var(--surface);color:var(--text2);border:1.5px solid var(--border);font-size:12px;font-weight:700;cursor:pointer;transition:all .2s;white-space:nowrap" onclick="openAuth('login')">Iniciar Sesión</button>
 <button class="btn-publish" style="padding:8px 14px;font-size:12px;border-radius:30px;white-space:nowrap" onclick="openAuth('registro')">Registrarse</button>
 </div>
 <div id="topbarUser" style="display:none;gap:8px;align-items:center">
 <span id="topbarUserName" style="font-size:13px;font-weight:600;color:var(--text2);white-space:nowrap;max-width:100px;overflow:hidden;text-overflow:ellipsis"></span>
 <button class="logout-btn" onclick="doLogout()" style="white-space:nowrap">⏻ Salir</button>
 </div>
 </div>
 </div>
 <!-- Barra de búsqueda expandible en móvil -->
 <div id="mobSearchBar" style="display:none;padding:8px 12px;background:var(--surface);border-bottom:1.5px solid var(--border)">
 <div class="search-wrap" style="display:flex;width:100%">
 <span style="color:var(--text3)">🔍</span>
 <input type="text" placeholder="Buscar eventos, cursos..." id="searchInputMob" oninput="handleSearch(this.value)" style="width:100%">
 </div>
 </div>
...
.mi-carrusel .track {
 display: flex;
 animation: miScroll 25s linear infinite;
}
...
<div class="slider">
 ...
 <img src="Imagen1(607).jpg">
 <div class="overlay">
 <span class="category">Lugar de acondicionamiento físico</span>
 <h3 class="title">GYM VITAZONE</h3>
 </div>
 </div>
 </div>
 ...
 <div class="dash-slide">
 <div class="card-inner">
 <img src="Imagen2(607).jpg">
 <div class="overlay">
 <span class="category">Gym</span>
 <h3 class="title">ASHER GYM</h3>
 </div>
 </div>
 </div>
 ...
 <div class="dash-slide">
 <div class="card-inner">
 <img src="Imagen3(607).jpg">
 <div class="overlay">
 <span class="category">Fútbol</span>
 <h3 class="title">Torneo</h3>
 </div>
 </div>
 </div>
 ...
 <div class="dash-slide">
 <div class="card-inner">
 <img src="Imagen4(607).jpg">
 <div class="overlay">
 <span class="category">Natación</span>
 <h3 class="title">Alberca</h3>
 </div>
 </div>
 </div>
...
carrusel.addEventListener('mouseenter', () => {
 track.classList.add('pausa');
});

carrusel.addEventListener('mouseleave', () => {
 track.classList.remove('pausa');
});
</script>

<style>
/* Carrusel del dashboard (clases propias para no colisionar) */
.slider { overflow: hidden; width: 100%; position: relative; padding: 10px 0; }
.dash-track {
 display: flex;
...
.dash-slide .card-inner { position: relative; overflow: hidden; border-radius: 15px; }
.dash-slide img { width: 100%; height: 260px; object-fit: cover; display: block; }
@keyframes dashScroll {
 0% { transform: translateX(0); }
 100% { transform: translateX(-50%); }
}
.dash-track.pausa { animation-play-state: paused; }

/* Desvanecido en bordes */
.slider::before, .slider::after {
 content: ""; height: 100%; position: absolute; width: 60px; z-index: 2; pointer-events: none; top: 0;
}
.slider::before { left: 0; background: linear-gradient(to right, var(--bg), transparent); }
.slider::after { right: 0; background: linear-gradient(to left, var(--bg), transparent); }

@media(max-width:900px) {
 .dash-slide { min-width: 190px; }
 .dash-slide img { height: 190px; }
}
@media(max-width:480px) {
 .dash-slide { min-width: 150px; }
 .dash-slide img { height: 155px; }
}
</style>

<script>
const dashSlider = document.querySelector('.slider');
const dashTrack = dashSlider ? dashSlider.querySelector('.dash-track') : null;
if (dashTrack) {
 dashSlider.addEventListener('mouseenter', () => dashTrack.classList.add('pausa'));
 dashSlider.addEventListener('mouseleave', () => dashTrack.classList.remove('pausa'));
 /* Soporte táctil: pausar al tocar */
 dashSlider.addEventListener('touchstart', () => dashTrack.classList.add('pausa'), {passive:true});
 dashSlider.addEventListener('touchend', () => setTimeout(() => dashTrack.classList.remove('pausa'), 2000));
}
</script>
...
<br>
 ...
 <div class="file-input-wrap" id="cuImgWrap">
 <input type="file" id="cu-img-file" accept="image/*" onchange="previewImg(this,'cuPreview','cuImgWrap')">
 <span id="cuImgLabel">📁 Haz clic o arrastra una imagen aquí</span>
 <img id="cuPreview" class="file-preview" alt="preview">
 </div>
 <input class="form-input" id="cu-img" placeholder="O pega una URL de imagen" style="margin-top:6px">
 </div>
 </div>
 <div class="form-grid full">
 ...
 <tbody id="tbCursos">
 <tr>
 <td colspan="7" style="text-align:center;color:var(--text3)">Cargando…</td>
 </tr>
 </tbody>
 </table>
 </div>
 ...
 <div id="mEvImg" class="modal-img-container sin-imagen"></div>
 <div class="m-body">
 ...
 <div id="mCuImg" class="modal-img-container sin-imagen"></div>
 ...
 ? `<div class="ev-thumb"><img src="${escHtml(ev.Imagen_url)}" onerror="this.parentNode.outerHTML='<div class=ev-thumb-placeholder ${g}>${em}</div>'"></div>`
 : `<div class="ev-thumb-placeholder ${g}">${em}</div>`;
 ...
 const costoNum = parseFloat(cu.costo || 0);
 const costo = costoNum > 0 ? `💰 $${costoNum.toFixed(2)}` : '<span style="color:var(--green);font-weight:700">✅ Sin costo</span>';
 const thumb = cu.Imagen_url
 ? `<div class="cc-header"><img src="${escHtml(cu.Imagen_url)}" onerror="this.parentNode.className='cc-header sin-img ${g}';this.parentNode.innerHTML='${em}'"></div>`
 : `<div class="cc-header sin-img ${g}">${em}</div>`;
 ...
 `<div class="mc-thumb"><img src="${escHtml(c.url)}" onerror="this.parentNode.className='mc-thumb sin-img ${gradients[i%gradients.length]}';this.parentNode.innerHTML='${ic}'"></div>` :
 `<div class="mc-thumb sin-img ${gradients[i%gradients.length]}">${ic}</div>`;
 ...
 const costo = parseFloat(ins.costo||0) > 0 ? `$${parseFloat(ins.costo).toFixed(2)}` : '<span style="color:var(--green);font-weight:700">Sin costo</span>';
 ...
 if (!data.length) {
 tb.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text3)">Sin registros</td></tr>';
 return;
 }
 ...
 <td>${cu.fecha_fin||'—'}</td>
 <td>${parseFloat(cu.costo||0)>0?`$${parseFloat(cu.costo).toFixed(2)}`:'<span style="color:var(--green);font-weight:700">Sin costo</span>'}</td>
 <td>${cu.Imagen_url?`<img src="${escHtml(cu.Imagen_url)}" style="height:36px;border-radius:4px;object-fit:cover" onerror="this.style.display='none'">`:'-'}</td>
 ...
 ['cu-nombre', 'cu-fi', 'cu-ff', 'cu-req', 'cu-desc', 'cu-costo', 'cu-img'].forEach(id => document.getElementById(id).value = '');
 resetFileInput('cu-img-file', 'cuPreview', 'cuImgLabel', '📁 Haz clic o arrastra una imagen aquí');
 w.style.display = 'block';
 ...
 document.getElementById('cu-img').value = cu.Imagen_url || '';
 if (cu.Imagen_url) {
 const p = document.getElementById('cuPreview');
 p.src = cu.Imagen_url;
 p.style.display = 'block';
 } else {
 resetFileInput('cu-img-file', 'cuPreview', 'cuImgLabel', '📁 Haz clic o arrastra una imagen aquí');
 ...
 imagen_url: document.getElementById('cu-img').value
 };
 data.action = id ? 'editar_curso' : 'crear_curso';
 if (id) data.id = id;
 const res = await apiFetch(data, {
 imagen_file: document.getElementById('cu-img-file')
 });
 if (res.ok) {
 ...
 if (ev.Imagen_url) {
 imgDiv.classList.remove('sin-imagen');
 imgDiv.innerHTML = `<img src="${escHtml(ev.Imagen_url)}" onerror="this.parentNode.innerHTML='⚽';this.parentNode.classList.add('sin-imagen')">`;
 } else {
 imgDiv.classList.add('sin-imagen');
 imgDiv.innerHTML = '⚽';
 }
 ...
 const costoModal = parseFloat(cu.costo || 0);
 const costoEl = document.getElementById('mCuCosto');
 if (costoModal > 0) {
 costoEl.textContent = `$${costoModal.toFixed(2)}`;
 costoEl.style.color = '';
 } else {
 costoEl.textContent = 'Sin costo';
 costoEl.style.color = 'var(--green)';
 costoEl.style.fontWeight = '700';
 }
 ...
 // Imagen del curso en el modal
 const imgDiv = document.getElementById('mCuImg');
 if (cu.Imagen_url) {
 imgDiv.classList.remove('sin-imagen');
 imgDiv.innerHTML = `<img src="${escHtml(cu.Imagen_url)}" onerror="this.parentNode.innerHTML='🎓';this.parentNode.classList.add('sin-imagen')">`;
 ...
 btn = document.getElementById('collapseBtn');
 ...
 // Imagen demasiado pequeña – rechazar
 showToast(`❌ Imagen muy pequeña (${img.width}×${img.height}px). Mínimo ${MIN_IMG_W}×${MIN_IMG_H}px.`, 'error');
 input.value = '';
 const p = document.getElementById(previewId);
 if (p) { p.src = ''; p.style.display = 'none'; }
 return;
 }
 // Tamaño válido – mostrar preview
 const p = document.getElementById(previewId);
 ...
 </script> <!-- BOTTOM NAV (movil) -->
 <nav class="bottom-nav" id="bottomNav">
 <div class="bottom-nav-inner">
 <div class="bn-item active" id="bn-dashboard" onclick="navigate('dashboard',null);setBnActive('bn-dashboard')">
 <span class="bn-icon">🏠</span>
 <span class="bn-label">Inicio</span>
 </div>
 <div class="bn-item" id="bn-eventos" onclick="navigate('eventos',null);setBnActive('bn-eventos')">
 <span class="bn-icon">📅</span>
 <span class="bn-label">Eventos</span>
 </div>
 <div class="bn-item" id="bn-cursos" onclick="navigate('cursos',null);setBnActive('bn-cursos')">
 <span class="bn-icon">🎓</span>
 <span class="bn-label">Cursos</span>
 </div>
 <div class="bn-item" id="bn-contenido" onclick="navigate('contenido',null);setBnActive('bn-contenido')">
 <span class="bn-icon">📰</span>
 <span class="bn-label">Noticias</span>
 </div>
 <div class="bn-item" id="bn-menu" onclick="toggleSidebar()">
 <span class="bn-icon">☰</span>
 <span class="bn-label">Menú</span>
 </div>
 </div>
 </nav>
 ...
 if (!visible) document.getElementById('searchInputMob').focus();
 }
 ...
 // Sincronizar bottom nav cuando se navega desde el sidebar
 const _origNavigate = navigate;
 // Patch navigate para también actualizar bottom nav
 (function patchNavigate() {
 const orig = window.navigate;
 window.navigate = function(page, navEl) {
 orig(page, navEl);
 const map = { dashboard:'bn-dashboard', eventos:'bn-eventos', cursos:'bn-cursos', contenido:'bn-contenido' };
 if (map[page]) setBnActive(map[page]);
 };
 })();
 </script>
</body></html>